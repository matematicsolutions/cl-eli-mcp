...
from cl_eli_mcp.coverage import build_coverage
from cl_eli_mcp.server import cl_coverage


def _coverage():
    fn = getattr(cl_coverage, "fn", cl_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CL_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("cl_coverage" in line for line in lines), (
        "an audit entry was written but it does not name cl_coverage"
    )
