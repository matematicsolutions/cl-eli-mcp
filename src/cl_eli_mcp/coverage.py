...
SOURCE = 'datos.bcn.cl (BCN Linked Open Data SPARQL)'
...
_FAMILIES: list[dict] = [{'name': 'Legislation metadata', 'tool': 'cl_search_norms / cl_get_norm'}]

_GAPS: list[dict] = [{'id': 'CL-001', 'family': 'Legislation metadata', 'missing': 'No full-text law content - only metadata (title, number, dates, issuing body) is returned, not the operative articles.', 'fallback': 'Follow source_url to the BCN or LeyChile page.'}, {'id': 'CL-002', 'family': 'Legislation metadata', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
